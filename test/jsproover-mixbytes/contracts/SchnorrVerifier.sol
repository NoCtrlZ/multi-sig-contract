...
contract SchnorrVerifier {
    ...
    function verifySignatureAsPoints(
        ...
        uint256 _s,
        uint256 _e,
        alt_bn128.G1Point memory _generator,
        alt_bn128.G1Point memory _publicKey
    ) public
    view
    returns (bool) {
        alt_bn128.G1Point memory r_V = _generator.mul(_s).add(_publicKey.mul(_e));
        uint256 e_V = uint256(keccak256(abi.encodePacked(r_V.X, r_V.Y, _publicKey.X, _publicKey.Y, _hash)));
        return e_V == _e;
    }
}
