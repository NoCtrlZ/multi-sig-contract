...
contract EfficientCoinMixer {
    ...
    function transfer(
        uint256 _assetID,
        uint256[2] memory _numOfInsAndOuts,
        uint256[] memory _parametersArray, // total 2 + (2+2)*numIns + (2+2+1)*numOuts following the parameters listed below for each it or out
        ...
        bytes32[] memory _outputKeyExchangeData, // chunks of output encoded data

        //range proofs
        uint256[8] memory coords, // [A_x, A_y, S_x, S_y, commits[0]_x, commits[0]_y, commits[1]_x, commits[1]_y]
    ...
    function transfer_internal(Board memory b) internal returns(bool success) {
        ...
        for (i = 0; i < b.inputPublicKeys.length; i++) {
            ...
            hashToVerify = keccak256(abi.encodePacked(reusablePoints[1].X, reusablePoints[1].Y));
