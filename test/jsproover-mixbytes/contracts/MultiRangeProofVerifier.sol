...
contract MultiRangeProofVerifier {
    ...
    function verify(
        uint256[] memory commitments, // multiple of 2 items of Peddersen commitments
    ...
    function verifyInternal(
        ...
        alt_bn128.G1Point[] memory input,
        MultiRangeProof memory proof
    ) internal
    view
    returns (bool) {
    ...
    function sumScalars(uint256[] memory ys) internal pure returns (uint256 result) {
    ...
    function powers(uint256 base, uint256 firstPower, uint256 numberOfPowers) internal pure returns (uint256[] memory powers) {
    ...
    function timesAndConcat(uint256[] memory v, uint256[] memory xs) internal pure returns(uint256[m] memory result) { //twos, zs. twos.times(z).reduce()
