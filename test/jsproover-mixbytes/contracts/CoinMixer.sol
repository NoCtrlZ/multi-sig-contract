...
contract CoinMixer {
    ...
    function deposit(uint256 _assetID, uint256 _value, uint256[2] memory _publicKey) public payable returns (bool) {
    ...
    function transfer(
        uint256[2+4+2+4+1] memory _scalars, // [from_X, from_Y, to_0_X, to_0_Y, to_1_X, to_1_Y, index_0, index_1, schnorr_0_S, schnorr_0_E, schnorr_1_S, schnorr_1_E, assetID]
        bytes memory _exchangeData0,
        bytes memory _exchangeData1,
        //range proofs
        uint256[20] memory coords, // [input_x, input_y, A_x, A_y, S_x, S_y, commits[0]_x, commits[0]_y, commits[1]_x, commits[1]_y]
        uint256[10] memory scalars, // [tauX, mu, t, a, b]
        uint256[2*2*n] memory ls_coords, // 2 * n
        uint256[2*2*n] memory rs_coords  // 2 * n
    ...
    function merge(
        uint256 _assetID, uint256[2] memory _publicKey0, uint256[2] memory _publicKey1,
        uint256[2] memory _signature0,
        uint256[2] memory _signature1,
        uint256[2] memory _newPublicKey) public returns (bool success) {
        alt_bn128.G1Point[] memory reusablePoints = new alt_bn128.G1Point[](3);
    ...
    function verifyRangeProofForSplits(
        ...
        function withdraw(uint256[2] memory _publicKey, uint256 _assetID, uint256 _value, uint256 _blinding) public returns (bool success) {
